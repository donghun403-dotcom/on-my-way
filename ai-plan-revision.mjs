...
export async function createAiPlanRevision(input, { apiKey, model = "gpt-5.4-mini", fetchImpl = fetch, timeoutMs } = {}) {
  ...
  try {
    ...
    body: JSON.stringify({
      ...
      instructions: [
        ...
        "days는 월요일부터 일요일까지 정확히 7개입니다. 선택하지 않은 요일은 isRestDay=true, taskIndexes=[]로 두세요.",
  ...
  try {
    ...
    if (validateRevisionOutput(normalized, revision).length) {
    ...
    if (caught?.code === "AI_OUTPUT_INCOMPLETE_MAX_TOKENS") {
      caught.message = "수정한 조건으로 계획을 다시 만들지 못했어요. 기존 계획은 그대로 유지했어요.";
    }
