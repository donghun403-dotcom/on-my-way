    ...
    response = await fetchAiResponse("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({
        ...
        instructions: [
          ...
          "conditions.periodDays: 목표 기간을 일수로. 말로 쓴 기간도 환산하세요(하나=1, 둘=2, 셋=3: '30일 동안' → 30, '한 달' → 30, '두 달' → 60, '3개월' → 90, '반년' → 180). '10월 말까지'처럼 달성 날짜만 있으면 0으로 두고 날짜는 질문의 defaultValue로 다루세요. 말하지 않았으면 0.",
          "작동 예시 — 입력: '수요일하고 금요일 아침에 40분씩 운동하고 싶어요. 석 달 안에 5kg 감량이 목표예요.' → conditions: {\"availableDays\":[\"수\",\"금\"],\"sessionMinutes\":40,\"weeklyFrequency\":2,\"periodDays\":90}. 말한 요일 두 개를 빠뜨리지 않고 전부 담은 것, '석 달'을 90으로 환산한 것에 주목하세요.",
          "conditions를 채우기 전에 사용자의 문장을 다시 읽고, 언급된 요일·시간·기간이 하나도 빠지지 않았는지 확인하세요.",
