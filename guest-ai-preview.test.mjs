...
function generatedBlueprint() {
  return {
    ...
    taskTemplates: [
      { type: "ACTION", title: "잠재 고객 한 명에게 문제 인터뷰를 요청하기", sourceReference: "고객 인터뷰 노트", quantityOrRange: "고객 1명", durationMinutes: 15, completionRule: "메시지 한 건을 보내면 완료", time: "저녁" },
      { type: "ACTION", title: "잠재 고객 반응 한 줄 정리", sourceReference: "고객 인터뷰 노트", quantityOrRange: "응답 1건", durationMinutes: 15, completionRule: "기록 한 건을 남기면 완료", time: "저녁" },
      { type: "ACTION", title: "인터뷰 질문 5개 정리", sourceReference: "고객 인터뷰 노트", quantityOrRange: "질문 5개", durationMinutes: 15, completionRule: "질문 5개를 쓰면 완료", time: "저녁" },
...
function generatedPlanForProviderRequest(init) {
  ...
  JSON.parse(providerBody.input.slice(providerBody.input.indexOf("{")));
...
function constrainedPlanForProviderRequest(init) {
  ...
  const result = generatedBlueprint();
  const available = new Set(normalizedInput.availability.availableDays);
  result.taskTemplates = result.taskTemplates.map((item) => item.type === "ACTION" ? {
    ...item,
  ...
  result.days = result.days.map((day, index) => {
    if (!available.has(["월", "화", "수", "목", "금", "토", "일"][index])) {
      return { isRestDay: true, taskIndexes: [] };
    }
    return { isRestDay: false, taskIndexes: [index === 2 ? 2 : 0] };
  });
  return result;
}
  ...
  const variants = [
    { name: "invalid-json", code: "AI_OUTPUT_PARSE_FAILED", output: () => "{not-json" },
    {
      ...
      output: (init) => {
        ...
        invalid.days = invalid.days.map(() => ({ isRestDay: true, taskIndexes: [] }));
