...
test("AI 제공자가 응답하지 않으면 제한 시간 뒤 명시적인 504 오류를 반환한다", async () => {
  await assert.rejects(
    ...
    (error) => error.status === 504 && error.code === "AI_PROVIDER_TIMEOUT" && error.retryable === true,
  );
});
...
test("AI 제공자 네트워크 오류를 안전한 502 오류로 변환한다", async () => {
  await assert.rejects(
    ...
    (error) => error.status === 502 && error.code === "AI_PROVIDER_UNAVAILABLE" && error.retryable === true && !error.message.includes("secret"),
  );
});
