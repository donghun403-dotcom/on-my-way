...
기기 **SM-G977N (Android 12)**, 빌드 **run 30801874631 → 30802536167**.
...
| # | 항목 | 관측 |
| --- | --- | --- |
| C-1 | 앱이 뜨고 `data-app-ready` | **통과** — `https://localhost/app.html`, `appReady: "true"` |
| C-4 | `/api`가 로컬 서버에 가로채이는가 | **가로채이지 않는다** — `/api/health` 200 + `application/json` + `{"ok":true,"environment":"production",…}` |
| C-6 | 로그인 첫 걸음 | 1차 **실패**(삼성 인터넷 이탈) → `allowNavigation`에 서버 호스트 추가 후 **앱 안** |
| **C-7** | **번들 페이지에서 세션 쿠키가 실리는가** ★★ | **실린다** — 아래 |
| C-8 | `force-stop` 후 유지 | **통과** — 재시작 후 번들 페이지에서 다시 `kakao/expired` |
| C-D | `.md` 내보내기 | **통과** — `b8ac6871-….md` 199 bytes. 이름은 여전히 blob UUID |

## C-7 — 이 회차의 전부

**절차**: 로그인을 완주하면 WebView가 실서버 origin에 남는다(`authState: member`).
거기서 **번들 페이지로 되돌린 뒤**(`https://localhost/app.html`) `/api/auth/me`를 물었다.
...
| `document.cookie` | 빈 문자열 — `HttpOnly`라 정상 |

**되돌려서 묻는 것이 측정의 핵심이다.** 실서버 페이지에 있는 채로 물으면 동일 출처라
당연히 되고 아무것도 증명하지 못한다.

**대조를 하나 더 걸었다.** 방금 이동의 잔상일 수 있으므로 `am force-stop`으로 죽이고
다시 띄웠다. 앱은 번들 페이지(`https://localhost/`)에서 시작했고, 같은 요청이 다시
`kakao/expired`를 돌려줬다. **네이티브 잼이 세션을 영속화하고 cross-site로 실어 준다.**

## 스파이크의 기각 근거 셋이 전부 해소됐다

`docs/capacitor-spike.md` §1은 **가설 A(시스템 브라우저 + 일회용 코드 + 딥링크 복귀)를
기각**하면서 "쿠키를 손에 넣어도 쓸 곳이 없다"고 적었다. 근거가 셋이었다.

| 블로커 | 상태 | 근거 |
| --- | --- | --- |
| ① 상대 경로가 서버까지 가지 않는다 | **해소** | `apiUrl()`. C-4가 200 + JSON |
| ② 절대 URL로 고쳐도 403이다 | **해소** | `CapacitorHttp`가 `Origin`을 붙이지 않는다(1회차 B-5). 서버 변경 0곳 |
| ③ 쿠키를 주입해도 붙지 않는다 | **해소** | **C-7.** 네이티브 잼이 `SameSite=Lax` 쿠키를 cross-site 요청에 싣는다 |

**셋이 다 열렸으므로 기각 사유가 없다.** 번들 앱이 `server.url` 없이, 그리고 웹의 CSRF
방어를 낮추지 않고 성립한다. 스파이크가 우려한 "보안 자세를 두 군데 바꾸는" 거래는
치르지 않아도 된다.
...
**이 회차의 `allowNavigation`은 측정용이지 출시 형태가 아니다.** C-7을 재려면 세션을
잼에 앉혀야 했고, 그러려면 OAuth가 앱 안에서 끝나야 했다. 답이 나온 지금 출시 형태는
따로 정한다 — 콜백이 앱으로 돌아오게 하는 것(딥링크 복귀)이고, **C-7이 통과했으므로
그 경로가 실제로 동작한다는 근거가 생겼다.** 실패했다면 만들 필요조차 없었다.

## 다음

| 할 일 | 왜 |
| --- | --- |
| 딥링크 복귀 (커스텀 스킴 인텐트 필터 + 콜백 리다이렉트) | 로그인 뒤 번들로 돌아오게 한다. C의 마지막 조각 |
| 다운로드 파일 이름 | A′·B·C 전부 blob UUID. 파일은 나오므로 별건 |
| `server.url` 심사 위험 판단 | C가 서면 **감수할 이유가 없어진다** |

---

# 4회차 (구성 C 복귀) — 2026-08-03

기기 **SM-G977N (Android 12)**, 앱은 3회차와 같은 빌드(run 30809522337). 재빌드 없이
서버만 두 번 배포됐다 — PR #76(절대 URL + 복귀 허용목록), PR #77(바운스 페이지).

## 1차 시도 — 302 복귀는 로컬 서버가 거부한다

PR #76 배포 후 로그인 왕복이 `https://localhost/?auth=success`에서 멈췄다:

```
net::ERR_CONNECTION_REFUSED — 웹페이지를 사용할 수 없음
```

**서버 수정 자체는 동작했다** — 예전처럼 실서버 origin에 주저앉지 않고 번들 주소로
넘어왔다. 거부한 쪽은 Capacitor 로컬 서버다. 대조 셋이 원인을 가른다:

| 도착 방식 | 결과 |
| --- | --- |
| 서버의 **302 리다이렉트**로 도착 | **거부** — `ERR_CONNECTION_REFUSED` |
| 앱 안에서 직접 이동 | 통과 |
| 서버 페이지에서 **JS `location.replace`** | 통과 — 세션까지 정상 |

"쿠키는 딥링크가 필요 없다"(C-7)는 유지되지만, **내비게이션은 302로 못 돌아온다.**
그래서 서버가 셸 복귀일 때만 302 대신 **바운스 페이지**(자기 origin의 한 줄 HTML이
JS로 번들에 넘김)를 내리게 했다 — PR #77. `safeRedirectPath`가 이미 검증한 URL만
쓰므로 열린 리다이렉트 방어는 그대로다.

## 2차 시도 — 완주

PR #77 배포 후 같은 기기·같은 앱에서 왕복을 다시 밟았다. 카카오 세션이 WebView 잼에
남아 있어 자격증명 입력 없이 자동 완주됐다.

| 관측 | 값 |
| --- | --- |
| 문서가 로드된 URL (`performance.getEntriesByType('navigation')`) | **`https://localhost/app.html?auth=success`** |
| `/api/auth/me` | `kakao/expired` |
| `authState` | `member` |
| 최상위 액티비티 | 전 과정 `…verifyc/.MainActivity` — 앱 밖으로 나가지 않았다 |

`location.href`에는 쿼리가 없다 — 앱이 `auth=success`를 처리한 뒤 지운 것이고, 문서의
원래 로드 URL은 performance 항목이 증언한다.

## 구성 C 닫힘

`server.url` 없는 번들 앱에서 **전부 선다**:

| 조각 | 근거 |
| --- | --- |
| API가 워커에 닿는다 | 3회차 C-4 |
| 로그인이 앱 안에서 완주된다 | 3회차 C-6 + 이번 복귀 |
| 세션 쿠키가 cross-site로 실린다 | 3회차 C-7 |
| 재시작 후 세션 유지 | 3회차 C-8 |
| 로그인 뒤 번들로 돌아온다 | **이번 4회차** |
| `.md` 내보내기 | 3회차 C-D |

**출시 형태의 인증·데이터 경로가 검증됐다.** `server.url`(구성 A)의 심사 위험을 감수할
이유가 없어졌다. 셸 측정 루프는 여기서 끝난다.

## 남은 것 (전부 별건)

| | 무게 |
| --- | --- |
| 다운로드 파일 이름 (blob UUID) | 가벼움 — 파일·내용은 선다 |
| 퍼널 이벤트가 셸에서 CORS로 막힘 (`sendBeacon`은 CapacitorHttp가 못 가로챈다) | 가벼움 — fire-and-forget이라 흐름은 안 막지만 셸 퍼널 데이터가 빈다 |
| 10번 인쇄 제품 경로 (`ADMIN_PASSWORD` 필요) | 가벼움 — 인쇄 자체가 죽는 것은 이미 확정 |
| 프로덕션 appId·서명·AAB 릴리스 빌드 | **다음 단계** — 셸이 아니라 릴리스 체계의 몫 |
