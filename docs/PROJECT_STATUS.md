...
- 단위 테스트 503개 통과. 데스크톱·모바일 Chromium의 설치 인계·가격·온보딩 E2E 70개 통과, 기존 조건부 4개 건너뜀. Windows 샌드박스의 테스트 정리 지연으로 동일 범위를 정상 권한에서 재실행해 종료 코드 0을 확인했다. 홈페이지 표시를 브라우저에서도 확인했다. 운영 배포는 PR 검증 후 진행한다.
