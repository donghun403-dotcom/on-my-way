...
- 기준일: 2026-09-24 (KST, 유입 검토 추가; 과거 구현·배포 기록의 날짜는 유지)
...
## Google Play 등록정보 개선 실행 (2026-09-24)

- 사용자 전권 위임에 따라 공개 개인정보 처리방침의 결제 처리 안내를 Google Play 구매 검증 흐름으로 정정하고, 동기화 기기 식별값과 AI 응답 저장/외부 보존의 차이를 명시했다. 실제 수집·보존 동작이나 가격·체험 정책을 변경한 것은 아니다.
- 업로드용 문안 4개, 순서가 붙은 실제 UI 스크린샷 7장, 대체 텍스트, 데이터 보안 항목별 재검토표를 로컬 `brand/store/play-upload-20260924/`에 준비했다. PNG 7개 모두 1080×1920 RGB(알파 없음)를 확인했다. brand 산출물은 Git·웹 배포 제외다.
- 현행 공개 앱은 번들형이므로 홈페이지 배포가 설치된 앱까지 갱신한다는 가정을 배제했다. 마지막 성공 릴리스 빌드 `cb3ae517`의 앱 마크업과 새 이미지의 핵심 기능을 대조했다. 실제 공개 versionCode 대응은 Console 접근 후 확인해야 한다.
- Play Console을 다시 열었으나 현재 계정은 여전히 개발자 가입 화면이다. 기존 앱의 개발자 계정 로그인을 요청하고 계정 선택 화면을 남겼다. 신규 계정 생성·권한 확대·데이터 신고 임의 제출은 하지 않았다.
- 개인정보·대화 저장 계약 관련 기존 단위 검사 8개 통과. 공개 페이지의 데스크톱·모바일 화면 검사와 배포는 진행 중이며, 스토어 심사 제출·공개 반영은 아직 완료하지 않았다.

## Google Play 공개 등록정보 추가 점검 (2026-09-24)

- 한국어·대한민국 공개 페이지에서 설치 가능 표시, 전체 설명, 기존 스크린샷 4장(대화→하루 기록→오늘→계획), 새로운 기능, 데이터 보안과 지원 링크를 직접 확인했다. 로컬 새 이미지 8장은 아직 게시되지 않았다.
- 우선 수정: AI 자동 계획 생성이라는 출시 안내를 실제 수동 계획+AI 조정으로 정정, 체험 종료 시점·15크레딧·북 제외·자동 결제 구분 보완, 새 실제 UI 이미지 7장 후보로 교체. 캐릭터·체험 홍보만 있는 8번은 업로드 후보에서 제외했다.
- 데이터 보안의 “수집 없음”과 공개 방침·소스의 계정 및 목표 서버 저장이 맞지 않을 가능성이 높다. 수집/공유 정의와 공급자 처리 조건을 확인해 재신고할 필요가 있으며 기존 제출 초안을 그대로 재사용하지 않도록 주의 문구를 추가했다.
- [점검 결과와 Console 입력용 문안](play-store-review-2026-09-24.md)을 작성했다. 올바른 개발자 Console 접근은 미확인이라 게시·신고 변경은 하지 않았다. 공개 다운로드 구간만으로 신규 유입·전환 원인을 단정하지 않는다.
- 문서 및 로컬 업로드 지침 변경만 수행했다. 제품 코드 변경과 추가 테스트 없음.

## 출시 후 유입 경로 개선 — 실행 (2026-09-24)

- 사용자 승인에 따라 Instagram·Facebook 소개를 실제 앱 기능에 맞춰 저장했다. Facebook에는 출처 태그가 있는 Play 직접 설치 링크와 HTTPS 홈페이지 링크를 추가하고 저장 결과를 확인했다.
- Facebook 상단 “더 알아보기” 행동 버튼을 Play로 연결하고 저장값을 재확인했다. 소유자 SNS 인사이트 기준선은 로컬 `brand/marketing/account.md`에만 기록했다.
- 로그인 후 Instagram 최근 두 게시물의 전체 캡션에 이미 앱·다운로드 안내가 있음을 확인했다. 핵심 문제는 앱 안내 자체의 부재가 아니라 프로필·직접 링크 연결 및 오래된 운영 지침이다. 전체 게시물 18개, 팔로워 8명 확인.
- 홈페이지 한국어 가치 제안·첫 화면 Play 설치 링크, 네이티브 앱 내 설치 링크 숨김, 운영 도메인 canonical/OG/Twitter, 체험 기능 안내 일관성 수정.
- 로컬 brand 지침과 마케팅 플러그인 소스를 Phase 4로 갱신하고 1.0.1 설치 패키지를 재생성했다. brand는 기존 규칙대로 Git·배포 대상에서 제외된다. Cowork 설치 사본 교체는 미확인이다.
- 단위 테스트 503개 통과. 데스크톱·모바일 Chromium의 설치 인계·가격·온보딩 E2E 70개 통과, 기존 조건부 4개 건너뜀. Windows 샌드박스의 테스트 정리 지연으로 동일 범위를 정상 권한에서 재실행해 종료 코드 0을 확인했다. 홈페이지 표시를 브라우저에서도 확인했다.
- [PR #108](https://github.com/donghun403-dotcom/on-my-way/pull/108) 검증 완료: 전체 기기 E2E 838개 통과·기존 조건부 12개 건너뜀(2개 샤드), Cloudflare 미리보기 통합 검사 70개 통과. 2026-09-24 `71bf8c3`으로 main에 병합했다. [운영 배포 run 35943917007](https://github.com/donghun403-dotcom/on-my-way/actions/runs/35943917007)은 배포 후 검증까지 성공했다.
- 운영 홈페이지 HTTP 200, 한국어 가치 설명·Play 설치 링크·운영 canonical·북 제외 체험 안내 반영을 확인했다. 브라우저에서 설치 버튼을 직접 눌러 `com.olivenrich.onmyway`의 실제 Play 페이지로 이동함을 확인했다. 기존 Workers 공유 URL과 “Pro와 동일” 문구는 운영 HTML에서 제거됐다.
- 이 단계의 적용 가능한 진입 경로 개선은 완료했다. 운영 성과 증가 여부는 후속 기간 실측이 필요하다. 아래 기기·계정 접근 대기 항목을 완료로 표시하지 않는다.
- 외부 조치 대기: Instagram 외부 링크는 모바일 앱 전용 편집이라 사용자에게 추가 경로를 전달했다. Play Console은 연결 계정 모두 가입 화면을 표시해 기존 개발자 계정 로그인이 필요하다. 출시 설명 대체 문안을 준비했다.
- 실행 상세·남은 입력: [유입 검토 및 실행 기록](acquisition-review-2026-09-24.md).

## Google Play 출시 후 유입 검토 완료 (2026-09-24, 아래는 실행 전 기록)

- **SNS 추가 검토:** 사용자 제공 Instagram·Facebook 공개 프로필 확인. 팔로워는 각각 8·2명이며, 공개 최신 게시일은 9/8·9/9다. Instagram 공개 화면에는 앱 설명·외부 링크가 보이지 않고, Facebook은 캐릭터 소개와 홈페이지 링크가 있다. 사용자는 유료 광고 미집행을 확인했다. 상세 릴스·전체 캡션·현재 인사이트는 로그인 요구와 브라우저 오류로 미확인이다.
- **종합 진단 보완:** 기존 `brand/marketing/strategy.md`는 Phase 1에서 앱 노출·다운로드 안내를 배제하고 링크 클릭·가입을 측정하지 않도록 되어 있다. 현재 앱 유입 목표와 이 전략의 불일치가 핵심 개선 대상이다. 원본 전략·캐릭터·SNS 설정을 바꾸지 않고, 앱 공개·프로필 연결·실제 사용 시연·채널별 측정의 검토안을 보고서에 추가했다. 공개 관찰은 `brand/marketing/account.md`에도 날짜를 구분해 기록했다.
- 사용자 보고: Google Play 등록 완료, 신규 유입 없음. 이번 단계는 원인 검토와 개선 우선순위 정리만 수행했다.
- 공개 Play 페이지와 운영 홈페이지 HTTP 200 확인. 스토어는 `On My Way - 목표 실행 코치`, `10+ 다운로드`를 표시한다. 이 누적 구간으로 테스트 설치·실제 신규 유입·활성을 구분할 수 없으며 Play Console 실측치는 미확인이다.
- 확인된 문제: 스토어 본문의 수동 계획 안내와 새로운 기능의 AI 자동 변환 약속 불일치, 홈페이지 체험 기능 안내의 모순, 운영 canonical의 예전 Workers 도메인 참조, 채널·고유 사용자·재방문을 판별하기 부족한 현재 이벤트 집계. 관리자 KPI·리텐션은 샘플이며 실제 수치가 아니다.
- 기존 웹→Play 인계는 공개 JavaScript에서도 확인했다. 이미 구현된 기능을 다시 제안하거나 수정하지 않았다. 로컬 최신 스토어 이미지의 실제 게시 여부는 미확인이다.
- 권장 다음 단계는 한 타깃·한 채널의 첫 유입 실험이다. 먼저 Play Console 방문·클릭·설치를 분리해 병목을 확인한다. 포지셔닝, 첫 사용 부담, 짧은 체험의 영향은 실측 전 가설로 유지한다.
- 상세 근거·문안·검증 순서: [유입 검토 보고서](acquisition-review-2026-09-24.md).
- 제품 코드·정책·스토어·배포 변경 없음. 브라우저 연결 실패로 실제 앱 상호작용은 미검증이며 공개 HTTP 응답과 소스·로컬 이미지 검토로 범위를 제한했다. 문서 변경만 수행해 제품 테스트는 실행하지 않았다.

## 런처 아이콘 반영 — 업로드 파일이 vc2로 바뀌었다 (2026-08-04)

PR #83 + run 30835022056. 지금까지의 AAB는 런처 아이콘이 **Capacitor 기본 로봇**이었다
— 네이티브 프로젝트를 커밋하지 않아 `cap add`가 매번 되살렸다. patch-download와 같은
방식의 `patch-launcher-icons.mjs`가 cap add 직후 커밋된 산출물(`mobile/launcher-res/`,
밀도 5종 × PNG 4종 + 어댑티브 XML 2종)로 갈아끼운다. 원본(`brand/`)이 git 밖이라
생성은 로컬(`make-launcher-icons.mjs`), 무결성은 테스트 5건이 잠근다.

어댑티브 구성: 전경 마스코트(안전 영역 61%) + 파스텔 그라데이션 배경 PNG(기본 흰
단색 `@color`를 `@mipmap` 참조로 교체), 레거시 512 타일, 라운드 원형 크롭.

**`on-my-way-0.1.0-vc2.aab`(12.4MB)가 새 업로드 파일이다** — AAB를 직접 열어
`mipmap-*-v4/`의 배경 PNG 5종과 어댑티브 XML의 `@mipmap` 참조, 레거시 아이콘 픽셀까지
확인했다. Play 첫 업로드는 vc1이 아니라 이걸 쓴다(스토어 목록 512 아이콘과 기기
런처가 일치). vc1은 기록으로만 남는다.

같은 날 스토어 그래픽(PR #81)·Play Billing 서버 기반(PR #82)도 머지됐다 — 아래 항목.
...
`server.url` 없이 번들 에셋 + 기본 origin(`https://localhost`) + 절대 URL API +
`CapacitorHttp` + **`CapacitorCookies` 켬**.

### ① 위장을 버리니 로컬 서버 가로채기가 사라졌다

`/api/health`가 프로덕션 워커에서 **200 + `application/json`**으로 돌아온다. B를 깨뜨렸던
가로채기(B-4)가 C에는 없다. B의 `hostname` 위장이 문제를 풀지 않고 **만들고 있었다**는
판단이 확인됐다. 서버 변경 0곳이다.

### ② 쿠키가 실린다 — 이 회차의 전부

로그인을 완주하면 WebView가 실서버 origin에 남는다. 거기서 **번들 페이지로 되돌린 뒤**
(`https://localhost/app.html`) `/api/auth/me`를 물었다.

| | 값 |
| --- | --- |
| 페이지 origin | `https://localhost` |
| API origin | `https://onmyway.olivenrich.com` |
| `/api/auth/me` | **200, `user: kakao/expired`** |

되돌려서 묻는 것이 핵심이다 — 실서버 페이지에서 물으면 동일 출처라 아무것도 증명하지
못한다. **대조를 하나 더 걸었다**: 방금 이동의 잔상일 수 있으므로 `am force-stop`으로
죽이고 다시 띄웠고, 번들 페이지에서 시작한 앱이 같은 답을 돌려줬다.
**네이티브 잼이 세션을 영속화하고 cross-site로 실어 준다.**

### ③ 그래서 스파이크의 결론이 뒤집힌다

`docs/capacitor-spike.md` §1은 가설 A(시스템 브라우저 + 딥링크 복귀)를 기각하며
"쿠키를 손에 넣어도 쓸 곳이 없다"고 적었다. 근거 셋이 **전부 해소됐다.**

| 블로커 | 근거 |
| --- | --- |
| ① 상대 경로가 서버까지 가지 않는다 | **해소** — `apiUrl()`, C-4가 200 + JSON |
| ② 절대 URL로 고쳐도 403이다 | **해소** — `CapacitorHttp`가 `Origin`을 붙이지 않는다(1회차 B-5) |
| ③ 쿠키를 주입해도 붙지 않는다 | **해소** — 이번 C-7 |

**번들 앱이 `server.url` 없이, 웹의 CSRF 방어를 낮추지 않고 성립한다.** 스파이크가
우려한 "보안 자세를 두 군데 바꾸는" 거래는 치르지 않아도 된다. `server.url`의 심사
위험을 감수할 이유도 없어진다.

### 남은 구조적 문제 — 로그인 뒤 앱이 실서버에 주저앉는다

`allowNavigation`에 서버 호스트를 넣었으므로 콜백이 WebView 안에서 끝나는데 **끝난
자리가 실서버 페이지다.** 그 순간부터 사실상 구성 A이고 번들의 의미가 없어진다.

**이번 `allowNavigation`은 측정용이지 출시 형태가 아니다.** C-7을 재려면 세션을 잼에
앉혀야 했다. 출시 형태는 딥링크 복귀이고, **C-7이 통과했으므로 그 경로가 동작한다는
근거가 생겼다** — 실패했다면 만들 필요조차 없었다.

다운로드도 통과했다(199 bytes). 이름은 A′·B·C 전부 blob UUID로 떨어진다.

## Capacitor 셸 2회차 — 1회차의 최대 차단이 뚫렸고, 새 차단 하나를 우리가 만들고 있었다 (2026-08-03)

기기 **SM-G977N (Android 12)**, 빌드 **run 30780890784**, 구성 A′·B 동시 설치.
이번에는 케이블이 데이터를 물어 **USB로 붙었다**(1회차는 무선 디버깅이었다).

### ① `allowNavigation` 한 줄로 provider 3종이 전부 앱 안에 남았다

1회차에서 카카오·네이버·구글 셋 모두 삼성 인터넷으로 이탈했던 것이 뒤집혔다. 최상위
액티비티가 셋 다 `…verifya/.MainActivity`였다(화면이 아니라 `dumpsys`로 판정).

| provider | 1회차 | 2회차 (A′) |
| --- | --- | --- |
| 카카오 | 삼성 인터넷 이탈 | **앱 안** — `accounts.kakao.com/login` |
| 네이버 | 삼성 인터넷 이탈 | **앱 안** — `nid.naver.com/oauth2.0/authorize` |
| 구글 | 삼성 인터넷 이탈 | **앱 안** — `accounts.google.com/v3/signin/identifier` |

**예상이 틀렸다.** 구글이 `disallowed_useragent`로 막을 것으로 적어 뒀는데 로그인 폼이
그대로 떴다. UA에 `; wv)`가 **들어 있는 채로** 통과했으므로 우리를 못 알아봐서가 아니다.
다만 잰 것은 식별 단계 화면이 렌더된다는 것뿐이고 **완주는 미검증**이다 — 자격증명 입력이
필요해 이번 세션에서 진행하지 않았다. 이 경로의 안정성은 우리가 통제하지 못한다.

**와일드카드 판단이 값을 했다.** 카카오가 `kauth` → `accounts`로 리다이렉트했다.
`kauth.kakao.com`만 적었다면 거기서 앱을 벗어났고, 관측은 "WebView에서 OAuth가 안 된다"는
틀린 결론으로 기록됐을 것이다.

### ② 다운로드 — 네이티브 패치는 동작한다. 막고 있던 것은 우리 CSP였다

| | A′ | B |
| --- | --- | --- |
| 파일 생성 | **실패** | **통과** — `/sdcard/Download/…md` 83 bytes |
| 이름 보존 | 측정 불가 | **실패** — blob UUID |
| 내용 | 측정 불가 | **통과** |

B가 끝까지 돈 것이 증거다. **1회차의 실패 원인(`DownloadListener` 부재)은 닫혔다.**

A′만 실패한 이유는 `worker.mjs`의 `connect-src`에 `blob:`이 없어서다. `'self'`는 blob
스킴을 덮지 않으므로, 패치가 주입한 읽기 조각의 `fetch(blob:…)`가 CSP로 거부됐다. B는
로컬 서버가 페이지를 줘서 그 헤더가 붙지 않고, 그래서 통과한다 — **변수 하나만 다른
대조에서 결과가 뒤집혔으므로 원인 판정은 닫힌다.**

**웹에는 증상이 없다.** 브라우저의 `<a download>`는 blob을 `fetch`하지 않고 직접
내려받는다. 네이티브 경로에서만 blob을 읽어야 해서 드러난 결함이다.

**고쳤다**: `connect-src`에 `blob:` 한 토큰을 넣고 `auth-service.test.mjs`가 그것을 지키게
했다. blob URL은 그 페이지가 스스로 만든 것만 열 수 있어 외부로 나가는 통로가 아니다.
테스트가 실제로 잡는지 일부러 빼서 확인했다(`not ok 56`).

**그리고 배포 후 다시 재서 확인했다 (PR #74 머지 → 프로덕션 배포 → 같은 기기).**
기기부터 재면 실패했을 때 원인이 CSP인지 배포 지연인지 갈리지 않으므로 **프로덕션 응답
헤더를 먼저 확인**했고(`connect-src 'self' blob: https://*.tosspayments.com`), 그다음
앱을 재시작해 `exportRecordsFile()`을 불렀다.

| | 배포 전 | 배포 후 |
| --- | --- | --- |
| A′ 파일 생성 | 실패 | **통과** — `/sdcard/Download/d08a2d6e-….md` 152 bytes |
| logcat | `blob 읽기 실패: TypeError: Failed to fetch` | `저장 완료: d08a2d6e-….md (152 bytes)` |
| 내용 | — | **회원 기록이 실제로 들어 있다** (목표 1줄 + `## 2026-08-01` 항목) |

**진단이 끝에서 끝까지 확인됐다.** 이번엔 로그인된 `kakao` 계정으로 쟀기 때문에 파일이
152 bytes이고 실제 데이터가 들어 있다 — B의 83 bytes는 익명 상태의 빈 기록이었다.
**빈 파일이 아니라 실제 기록이 나온다는 것이 여기서 처음 확인됐다.**

남은 결함 하나: **이름이 blob UUID로 떨어진다.** `blob:`에는 `Content-Disposition`이 없어
`URLUtil.guessFileName`이 URL 조각을 쓴다. A′·B 양쪽 모두 그렇다. 파일은 생기고 내용도
열리므로 열람·이동권은 서고, 이름은 다음 단이다.

### 페이월에 대해 — 근거 하나가 사라졌을 뿐이다

1회차가 세운 **"`.md`가 나오지 않으므로 `HARD_PAYWALL_ENABLED`를 켤 수 없다"는 차단은
해소됐다.** 잠긴 유저가 자기 기록을 가져갈 경로가 앱에 실재한다는 것을 확인했다.

**그렇다고 켜도 된다는 뜻은 아니다.** 선행 조건은 원래 세 개였고 나머지 둘이 그대로다 —
`PAYMENTS_ENABLED=true`와 실결제 검증. 그쪽이 훨씬 무겁고, 결제자가 0인 상태에서 켜면
유저 100%가 잠긴다는 2026-07-30의 운영 숫자도 그대로다.

### ③ 측정 방법 정정 — `find /sdcard`는 파일이 있어도 못 찾는다

체크리스트가 지정했던 `adb shell find /sdcard -iname '*.md'`가 파일이 실재하는데도 빈
결과를 냈다. 같은 시점 `ls /sdcard/Download/*.md`는 보여 준다. 1회차 9번의 실패 판정은
근거가 셋이었고 나머지 둘이 독립적으로 같은 결론을 가리켰으므로 **유지된다.** 다만
`find` 하나만 봤다면 거짓 음성이었다. 판정 방법을 `ls`와 MediaStore 조회로 바꿨다.

### ④ 로그인이 완주되고 세션이 선다 — 제품 코드 수정 0곳으로

사용자가 폰에서 카카오 로그인을 완주한 뒤 이어서 쟀다.

| # | 결과 |
| --- | --- |
| 7 | **통과** — `/api/auth/me` 200 `{"user":{…}}`, `usr_jnqNBEje…` / `kakao` / plan `expired`, `authState: "member"`. `document.cookie`는 빈 문자열(`HttpOnly`라 정상) |
| 8 | **통과** — `am force-stop` 후에도 세션 유지, `localStorage` 11키 그대로 |

1회차에서 구성 A가 "가장 나쁜 형태의 성공"(뜨지만 로그인 불가)이었던 것이 닫혔다.
**8번은 1회차와 성격이 다르다** — 그때는 "없던 것이 여전히 없다"를 본 것이고, 이번에는
세션이 실재하는 상태에서 죽였다 살렸으므로 유지 여부가 실제로 갈렸다.

측정 경로를 정직하게 적으면: 자격증명은 사용자가 직접 입력했고, 이 세션은 그 뒤
`/api/auth/kakao/start`를 다시 밟아 **자격증명 없이 왕복이 완주되는 것**을 관측했다.
OAuth 왕복 전체가 WebView 안에서 끝나고 세션 쿠키가 같은 잼에 앉는다는 것이 확인된 것이다.

### 남은 것

**10(인쇄 제품 경로)만 미검증이다.** 계정이 `expired`이고 Pro 승격은
`/api/admin/users/update` → `role === "admin"` 세션 → `ADMIN_PASSWORD`를 요구한다.
비밀번호 입력이 필요한 단계라 진행하지 않았다. 우선순위는 낮다 — 1회차에서
`window.print()`가 신호 없이 죽는 것이 이미 확정됐고, 10번이 추가로 답하는 것은
`document.title`이 갇히지 않는지 하나다.

**다음 회차 후보**: ⓐ CSP 배포 후 A′에서 D를 다시 재기 ⓑ 다운로드 파일 이름 보존
ⓒ 네이버·구글 완주 ⓓ `server.url` 없이 서는 구성(심사 위험 때문에 A는 상한선 측정용이다).
