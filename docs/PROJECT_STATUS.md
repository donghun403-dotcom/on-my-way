...
- Google은 실제 로그인·세션·계정 정책 시나리오를 확인했으며 로그인 취소 안내만 아직 미확인이다. Kakao는 실제 계정 로그인과 OAuth callback 복귀까지 확인했고 나머지 세션·계정 정책 시나리오는 아직 미확인이다.
- Naver는 실제 로그인, 세션 유지, 로그아웃 후 데이터 비노출, 동일 계정 복원과 Google·Kakao·Naver 데이터 격리를 확인했다. Apple 실제 Provider 검증은 실패가 아니라 iOS 출시 단계로 연기했다.
...
- Kakao는 실제 로그인과 callback 복귀까지만 확인됐고 Naver는 아래 시나리오까지 확인됐다. Android 인증 완료 판정은 Google 로그인 취소와 Kakao·Naver 잔여 항목 확인 후에만 한다.
...
- Naver는 아래 시나리오까지 확인됐고 Apple의 실제 Provider 검증은 iOS 출시 단계로 연기했다.
...
| -- | -- | ----- | -- | ----- | -------- | ----- |
| A1 | 1단계 — 인증 안정화 | Kakao·Naver·Google 가입/로그인, 세션 유지, 로그아웃 | 진행 중 | Google은 실제 로그인·세션·계정 정책을 확인했고 취소 안내만 남았다. Kakao는 실제 로그인·callback 복귀를 확인했고 Naver는 로그인·세션·로그아웃·재로그인·3 Provider 격리를 확인했다. Kakao와 Naver의 명시된 잔여 수동 항목은 미확인이다. | `auth-service.mjs`, `auth-service.test.mjs`, PR #9 | Android 대상 세 Provider의 잔여 수동 항목만 검증 |
...
| A3 | 1단계 — 인증 안정화 | 계정 격리, 탈퇴, 재로그인 정책, 인증 실패 처리와 테스트 | 진행 중 | 공통 서버 계정·탈퇴·계정별 저장소 격리와 회귀 테스트가 존재한다. Google은 주요 계정 정책을 확인했지만 Google 취소 안내와 Kakao·Naver 잔여 실제 시나리오는 남아 있다. | `auth-service.mjs`, `worker.mjs`, `auth-service.test.mjs`, `apple-auth.test.mjs`, `tests/e2e/storage-recovery.spec.js`, PR #9 | Android 대상 세 Provider의 잔여 수동 항목 검증 |
...
필요한 수동 검증: Google 로그인 취소 안내, Kakao 세션 유지·로그아웃·동일 계정 복원·Google 격리·앱 내부 탈퇴·세션 무효화·삭제 대기·취소, Naver 앱 내부 탈퇴·세션 무효화·삭제 대기·취소. Kakao unlink와 Naver 외부 연결 해제 API는 Android 1차 출시 후속 정책으로 추적할 수 있지만 앱 내부 탈퇴·세션 폐기·데이터 삭제는 출시 전에 확인한다.
...
1. Android 인증 게이트에는 Google 로그인 취소 안내, Kakao 잔여 세션·계정 정책, Naver 앱 내부 탈퇴·세션 무효화·삭제 대기·취소 검증이 남아 있다.
...
PR #9 Preview에서 Google 로그인 취소 안내와 Kakao·Naver의 위 Android 잔여 인증 시나리오를 실제 계정으로 확인한다. 미확인 항목이 남으면 PR #9을 Ready로 전환하거나 병합하지 않는다.
