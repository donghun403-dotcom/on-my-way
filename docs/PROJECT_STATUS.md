...
- E2E: 손댄 스펙(onboarding · cheer · plan · responsive) 전부 통과. 전체 스위트는 이 머신에서 항상 1건 안팎의 타임아웃 플레이크가 남는다 — 손대지 않은 `origin/main`도 같은 머신에서 5건 실패하므로 브랜치 귀속이 아니다. **CI(2 workers, retries=2) 재확인이 병합 조건**이다.
