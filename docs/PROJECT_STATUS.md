...
- **무료 치어링 하루 1회 상한(W2).** `claimDailyCheer`/`releaseDailyCheer`가 KV read-modify-write에 isolate 내부 락만 걸려 있어 colo 간 상호배제가 없다. 상한이 묶는 것이 provider 호출 = 실제 AI 비용이므로 실질 상한이 없는 상태다. 유저당 EnergyLedger DO가 이미 있으니 그 안에 일일 클레임을 두는 것이 새 DO보다 싸다.
