...
**2026-08-04 갱신 — §2 스키마와 수신·검증 경로가 1회차로 구현됐다.**
`migrations/0002_store_entitlements.sql`(초안 §2 그대로) + `entitlement-service.mjs`
(RTDN 멱등 수신, subscriptionsv2 검증, acknowledge) + 라우트 2개(구성 없으면 503).
**§5(지급 키·환불 정책)와 §6(원장 경계)은 여전히 구현하지 않았다** — 그 결정들은
실결제 검증 라운드에서 명시적으로 내린다. 아래 본문은 설계 근거 기록으로 그대로 둔다.
...
## 5. 지급 원칙 (설계만 — 구현하지 않았다)

**근거**: 결제 → 250 크레딧 소진 → 환불 → 재구매 → 다시 250은 **체험 크레딧 무한 리필
(#40)과 같은 형태다.** 그때도 지급을 부르는 경로는 정당했고, 아무도 "이미 준 적이
있는가"를 묻지 않았다. 구독에서 그 질문을 어디서 던질지 먼저 정한다.

### 5.1 지급 키

**지급 키 = `store_subscription_id` + 구독 주기.**

| 상황 | 지급 |
| --- | --- |
| 새 구독 (새 `store_subscription_id`) | **지급** |
| 갱신 (같은 구독의 다음 주기) | **지급** — 주기가 다르다 |
| 같은 구독의 **재개** (`grace` → `active`, `on_hold` → `active`, `paused` → `active`) | **재지급 없음** — 같은 구독, 같은 주기다 |
| 같은 알림의 중복 전달 | **재지급 없음** — `store_notifications`의 UNIQUE가 앞에서 막는다 |

**"재개는 재지급 없음"이 이 키의 존재 이유다.** 상태 전이만 보고 지급하면 결제가 한 번
실패했다 복구된 유저가 그 주기에 두 번 받는다. `grace`는 **권한이 유지되는** 상태라
(§3) 그 사이에도 크레딧을 쓸 수 있어서, 재지급되면 한 주기에 500이 된다.

**구현 시 원장이 이 키를 기록해야 한다.** 지금 원장에는 `lastGrantMonthKey`가 있고
(**[코드]** `energy-ledger.mjs`의 `RESET_DISPOSITION`에서 `rebuilt`) 체험은
`trialGrantKey`로 따로 잡는다(`preserved`). 구독 지급 키도 같은 자리에 같은 방식으로
들어가야 하고, **`RESET_DISPOSITION`에 처분이 선언돼야 한다** — `CONTRIBUTING.md` ④.
`rebuilt`로 두면 플랜 전환 한 번에 지급 이력이 지워져 #40이 그대로 재현된다.

**[확인 필요]** 구글의 업/다운그레이드는 **새 `purchaseToken`을 발급**한다(§1). 지급 키가
`store_subscription_id`를 포함하므로 요금제를 바꾸면 **새 구독으로 보여 즉시 재지급된다.**
`linked_prior_subscription_id` 체인을 지급 판정에서 볼 것인지가 구현 시점의 결정 사항이다.
보지 않으면 "업그레이드 → 다운그레이드"를 반복해 크레딧을 무한히 뽑을 수 있다 —
**이것도 #40과 같은 형태다.**

### 5.2 지급 전에 환불 이력을 조회한다

**지급 경로는 `entitlement_refunds`를 반드시 읽는다.** §2의 전용 표와
`idx_entitlement_refunds_user` 인덱스가 그 조회를 위해 있다.

읽은 뒤 무엇을 할지는 **구현 시점에 명시적으로 정한다.** 지금 답을 적어 두지 않는 이유는,
이 결정이 "얼마를 잃을 것인가"의 선택이지 기술 문제가 아니기 때문이다. 다만 **묻지 않고
넘어가는 것만은 막는다** — #40이 그렇게 생겼다.

**정해야 할 질문 (직전 주기가 환불된 경우)**

| # | 질문 | 선택지 |
| --- | --- | --- |
| 1 | 재구매에 **전액 지급**하는가 | ① 전액 — 정상 유저의 실수·카드 문제까지 벌하지 않는다. 대신 남용에 열려 있다 ② 직전 주기에 **쓴 만큼 차감**해서 지급 ③ 지급하지 않음 |
| 2 | 환불 시점에 **남은 크레딧을 회수**하는가 | 회수하면 잔량이 음수가 될 수 있다(이미 썼으므로). 두면 환불받고도 그 달 크레딧을 다 쓴다 |
| 3 | 몇 번까지 봐주는가 | "직전 1주기"만 보는가, 계정의 **누적 환불 횟수**를 보는가. 후자면 체험 남용 마커(`ai-credits-service.mjs:544`)와 같은 모양이 된다 |
| 4 | 환불이 **지급보다 늦게** 도착하면 | 스토어 통보는 지연된다. 이미 지급한 뒤 환불 통보가 오는 순서를 어떻게 다루는가 |

**4번이 가장 놓치기 쉽다.** 1~3은 "지급할 때 환불을 본다"로 풀리지만, 4번은 반대 방향이라
지급 경로만 고쳐서는 닫히지 않는다.

**[확인 필요]** 환불 정책을 문서화하면 스토어 등록 정보와 이용약관에도 반영해야 한다.

## 6. 원장(EnergyLedger)과의 경계 — 설계하지 않은 것
