...
- `PAYMENTS_ENABLED=false`를 유지하고 Toss key·Sandbox 결제·Production D1은 설정하지 않았습니다.
...
- Preview Worker Secret `TOSS_CLIENT_KEY`와 `TOSS_SECRET_KEY`는 현재 미설정입니다. 키 값, MID 전체값, billing key, paymentKey, Secret은 로그·응답·문서에 남기지 않습니다.
...
- ACTION REQUIRED: Toss 개발자센터에서 자동결제 계약 상태 또는 개발 연동 체험 상점의 Test MID와 키 세트를 사용자 측에서 확인하고, Cloudflare Preview Worker의 Secret에 두 이름만 등록합니다. `PAYMENTS_ENABLED=false`, `ALLOW_DEMO_BILLING=false`와 Production Worker는 변경하지 않습니다.
