...
사용자가 PRO를 선택하면 토스 SDK 카드 등록창이 열리고, 서버가 `authKey`로 빌링키를 발급한 뒤 첫 달 2,900원을 승인한다. 매일 00:15 UTC에 실행되는 Worker Cron이 결제일이 지난 활성 구독을 갱신한다. 사용자가 해지하면 빌링키를 삭제하고 이미 결제한 기간 종료일까지 PRO를 유지한다.
