...
- PR Preview workflow는 database 이름으로 D1을 확인하고 배포 직전에만 임시 Wrangler 설정에 `BILLING_DB` ID를 주입합니다. ID는 GitHub 로그에서 mask 처리하고 생성 설정은 정적 assets에서 제외하며, 저장소·PR·문서에는 기록하지 않습니다.
