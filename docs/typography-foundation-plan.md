...
있지만 **이 4개 화면에 실제로 렌더링되는 것만** 다룬다. 각각을 위 역할 표에 비추어
`--weight-display`(800) 또는 `--weight-title`(700)로 옮긴다.
...
4. **새 절 추가** — `--weight-*` 축을 토큰 목록에 넣고, **`900`이 237개 남아 있으며
   그 값들은 의도적으로 고른 것이 아니라는 사실**을 적는다. A·B 단계가 이어받는다.
...
적을 것: 무엇이 문제였는지, 왜 기존 테스트가 못 잡았는지, 무엇을 바꿨는지,
실측 수치(subset 개수·바이트), 그리고 남은 900 237개가 A·B로 넘어간다는 사실.
